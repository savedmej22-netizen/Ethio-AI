...
// ⭐ NEW: This is the route your frontend actually calls.
// Streams the response back as Server-Sent Events, matching the
// `data: {...}` / `data: [DONE]` format your App.tsx expects.
app.post("/api/chat/stream", async (req, res) => {
  ...
  try {
    const { messages, languagePreference } = req.body as {
    ...
    // Map frontend's {role: 'user'|'assistant', content} into Gemini's
    ...
    const languageNote =
      languagePreference === "amharic"
        ? "\n[Respond in Amharic using proper Ge'ez script.]"
    ...
    const stream = await ai.models.generateContentStream({
      ...
      config: {
        systemInstruction: SYSTEM_INSTRUCTION + languageNote,
        temperature: 0.7,
      },
    });
...
// Kept for compatibility / direct testing (non-streaming)
